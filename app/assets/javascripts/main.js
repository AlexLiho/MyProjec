...
var calculatePrice = function () {
    if (!$('select[name="order_item[product_id]"]').length) {
        return;
    }
    var additionalHtml = "";
    var total = 0;
    ...
    var flattype = parseInt($('select[name="order_item[product_id]"]').val());
    ...
    var windowNum = parseInt($('input[name="order_item_5[quantity]"]').val());
    ...
    var balNum = parseInt($('input[name="order_item_12[quantity]"]').val());
    total += bal * balNum;
    if (balNum) {
        additionalHtml += "<p>Мытье балконов " + balNum + "шт</p>";
    }
    var lustraNum = parseInt($('input[name="order_item_8[quantity]"]').val());
    total += lustra * lustraNum;
    if (lustraNum) {
        additionalHtml += "<p>Мытье люстр " + lustraNum + "шт</p>";
    }
    var holodNum = parseInt($('input[name="order_item_6[quantity]"]').val());
    total += holod * holodNum;
    if (holodNum) {
        additionalHtml += "<p>Мытье холодильника " + holodNum + "шт</p>";
    }
    var duhovNum = parseInt($('input[name="order_item_7[quantity]"]').val());
    total += duhov * duhovNum;
    if (duhovNum) {
        additionalHtml += "<p>Мытье духовки/СВЧ внутри " + duhovNum + "шт</p>";
    }
    var glad1Num = parseInt($('input[name="order_item_9[quantity]"]').val());
    total += glad[1] * glad1Num;
    if (glad1Num) {
        additionalHtml += "<p>Глажка " + glad1Num + "шт</p>";
    }
    var glad2Num = parseInt($('input[name="order_item_10[quantity]"]').val());
    total += glad[2] * glad2Num;
    if (glad2Num) {
        additionalHtml += "<p>Поменять бельё " + glad2Num + "шт</p>";
    }
    var glad3Num = parseInt($('input[name="order_item_11[quantity]"]').val());
